...
const updateList = function (e) {
  ...
  fetch("/updateList", {
    ...
    .then((json) => {
      ...
      let libList = document.getElementById("lib");
      libList.innerHTML =
        "<thead><tr><th>Book</th><th>Author</th><th>Date Started</th><th>Date Completed</th><th>Time Spent Reading</th><th>Remove?</th><tr></thead>";

      json.forEach((thing) => {
        libList.innerHTML =
          libList.innerHTML +
          `<tr><td>${thing.bookTitle}</td><td>${thing.authorName}</td><td>${thing.dateStart}</td><td>${thing.dateComp}</td><td>${thing.timeTook}</td></tr>`;
      });
    });
};
